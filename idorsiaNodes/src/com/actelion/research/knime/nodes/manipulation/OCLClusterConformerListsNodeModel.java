...
import com.actelion.research.knime.nodes.manipulation.OCLClusterMoleculesNodeModel.ClusterColumnCellFactory;
import com.actelion.research.knime.nodes.manipulation.OCLClusterMoleculesNodeModel.RepresentativeColumnCellFactory;
import com.actelion.research.knime.utils.DescriptorHelpers;
import com.actelion.research.knime.utils.SpecHelper;

/**
 * <code>NodeModel</code> for the "OCLClusterConformerLists" node.
 *
 * @author Idorsia Pharmaceuticals Ltd.
 */
public class OCLClusterConformerListsNodeModel extends NodeModel {
    ...
    protected OCLClusterConformerListsNodeModel() {
        super(1, 1);
    }
...
//        }
    ...
    protected BufferedDataTable[] execute(BufferedDataTable[] inData, ExecutionContext exec) throws Exception {
        ...
        while (iterator.hasNext()) {        	        
        	...
        	LOG.debug("OCLClusterConformerListsNodeModel::execute(..) -> extract data from row "+idx);
                    
            // handle PheSA..
            if(m_settings.getDescriptor().shortName.equals( DescriptorConstants.DESCRIPTOR_ShapeAlign.shortName )) {
            	// create PheSAMolecule from conformer list            	            
            	DataCell next = iterator.next().getCell(structureCellIdx);
            	OCLConformerListDataCell conformer_datacell = (OCLConformerListDataCell) next; 
            	ConformerSet conformerset = conformer_datacell.getConformerSet();            	            	            
            	descriptorList[idx++] = DescriptorHandlerShape.getDefaultInstance().getThreadSafeCopy().createDescriptor( conformerset );	
            }                                   
        }
        
        LOG.info("init clusterer..");
    ...
    private class RepresentativeColumnCellFactory extends SingleCellFactory {
        private Clusterer clusterer;
        private int currentRowIdx;
